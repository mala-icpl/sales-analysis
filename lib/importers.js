...
// placed in the last days of the prior month get invoiced/shipped early
// next month. Keying off invoice date keeps a monthly file's rows cleanly
// inside the month it was actually pulled for, so a re-upload of "July
// sales" replaces exactly the July batch.
//
// sales.period_key is NOT NULL in the database, so a row with no parseable
// invoice date can't be inserted at all. That's expected for orders placed
// but not yet invoiced (still processing/pending) — more likely to show up
// in a custom multi-month date-range export than a normal single-month
// one. Those rows are skipped (not force-fit into an approximate month)
// and reported back via `skipped`, so the caller can surface the count
// instead of the whole import failing or the gap going unnoticed;
// re-uploading the same file later, once those orders are invoiced, picks
